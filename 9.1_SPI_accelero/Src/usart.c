...
void dma1_init(uint32_t src, uint32_t dst, uint32_t len){
	...
	DMA1_Channel1 -> CCR |= (1U << 7) | (1U << 4) | (1U << 1) | (1U << 13);
	NVIC_EnableIRQ(DMA1_Channel1_IRQn);

}

void dma1_start(void){
	USART1 -> CR3 |= CR3_DMAT_EN;
	DMA1_Channel1 -> CCR |= CCR_EN;
}
...
void DMA1_Channel1_IRQHandler(void) {
    if (DMA1->ISR & ISR_TCIF1) {
        ...
        // Disable DMA to prevent retriggering
        DMA1_Channel1 -> CCR &= ~CCR_EN;

        // Optionally disable USART1 DMA TX
